...
describe("The \"exec\" method of the proxy", () => {
  ...
  describe("when it's called and the target callable returns with a resolving promise", () => {

    function targetCallable() {
      return new Promise(resolve => process.nextTick(() => resolve("aValue")));
    }

    const proxy = circuitBreaker.trap(targetCallable);

    it("should return a promise which resolves to the same value", () => {

      const promise = proxy.exec();

      expect(promise).to.be.an.instanceOf(Promise);

      return promise.then(result => expect(result).to.equal("aValue"));
    });
  });

  describe("when it's called and the target callable returns a with a rejecting promise", () => {
...
describe("The \"setThresholdNum\" method", () => {
  ...
  describe("when it sets the threshold number is the same like the remote call has already failed", () => {
  ...
  describe("when it sets the threshold number higher than the remote call has already failed", () => {
